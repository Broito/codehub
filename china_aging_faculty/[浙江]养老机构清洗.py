...
os.chdir(r'E:\workspace\Research_2021_7thCencus_AgingPeople\data_processing\长三角官方养老院数据\浙江')
df = pd.read_excel('cata_3636_总去重.xls','Sheet1')

df['full_address'] = df['机构住所'].map(lambda a: '浙江省'+str(a).strip('\n')) + df['养老机构名称']
...
df.to_csv("浙江养老机构_3636_带坐标.csv",encoding='GB18030')   
# gdf = gpd.GeoDataFrame(df,columns=df.columns,crs="EPSG:4326")
# gdf.to_file("浙江养老机构.shp",encoding='GB18030') 

# 江苏的有很多为reliability为1和3的，需要手动添加地级市县级市后再重新跑。 另外是大部分都重复了三遍，需要去重
# df2 = pd.read_csv(r'浙江养老机构.csv',encoding = 'GB18030')
...
#     print(index)
# df2_rlb.to_csv("浙江养老机构哦_低信度地理编码修改.csv",encoding='GB18030')  
