...
df = pd.read_csv('广东.csv',encoding = 'GB18030')

df['full_address'] = df['地址'].map(lambda a: '广东省'+a.strip('\n')) + df['单位名称']
...
df.to_csv("广东养老机构.csv",encoding='GB18030')   
gdf = gpd.GeoDataFrame(df,columns=df.columns,crs="EPSG:4326")
gdf.to_file("广东养老机构.shp",encoding='GB18030') 
