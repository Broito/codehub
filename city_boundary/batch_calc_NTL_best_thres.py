...
def find_best_thres(code6,name6,init_thres_step):
    ...
    # 若是阈值为1时，data_urban_ratio就小于ratio6了，那么直接以最小阈值1位最佳阈值
    if residual < 0:
        return 1,residual,pop6,urban6,urban_ratio6,data_total_pop,data_urban_pop,data_urban_ratio
    residual_list = []
    thres_list = []
    ...
    while True:
        ...
        # 类nppviirs最小就是1，round(1/2)即为0
        if thres_step == 0 or thres < 1:
            break
    ...
    return best_thres,best_residual,pop6,urban6,urban_ratio6,data_total_pop,data_urban_pop,data_urban_ratio
...
# %%
df = pd.read_excel(r'result_urban_pop.xls','Sheet1',encoding='gbk')
...
for i in lst_df:
    try:
        ...
        best_thres,best_residual,pop6,urban6,urban_ratio6,data_total_pop,data_urban_pop,data_urban_ratio = find_best_thres(code6,name,init_thres_step)
        copy_polygon_to_result(name,best_thres)

        df_output.loc[code6,'name'] = name
        df_output.loc[code6,'pop6'] = pop6
        df_output.loc[code6,'urban6'] = urban6
        df_output.loc[code6,'urban_ratio6'] = urban_ratio6
        df_output.loc[code6,'data_total_pop'] = data_total_pop
        df_output.loc[code6,'data_urban_pop'] = data_urban_pop
        df_output.loc[code6,'data_urban_ratio'] = data_urban_ratio
        df_output.loc[code6,'best_thres'] = best_thres
        df_output.loc[code6,'best_residual'] = best_residual
        ...
        print(f'{count}//{len(lst_df)},{code6},{name}-->prob!')
    count += 1
