...
def get_init_urban_patch(ori_patch,city_name):
    ...
    SpatialJoin_analysis(ori_patch,file_gov_buffer,file_joined_ori_urban,'JOIN_ONE_TO_MANY')

    # 按照buffer id 对斑块进行dissolve
    file_dissolved_ori_urban = f'{city_name}_dissolved_ori_urban.shp'
    Dissolve_management(file_joined_ori_urban,file_dissolved_ori_urban,'NAME')

    # 选择出NAME不是None的区域，即政府所在斑块
    file_init_urban_polygon = f'{city_name}_init_urban_polygon.shp'
    Select_analysis(file_dissolved_ori_urban,file_init_urban_polygon,"NAME <> ''")
    # 有一些区域是重叠的，需要删掉重叠的polygon
    DeleteIdentical_management(file_init_urban_polygon,'Shape')
...
def single_shp_to_multi(multi_shp,city_name):
    ...
    # 添加面积字段
    AddGeometryAttributes_management(multi_shp,'AREA')
    fields = ['SHAPE@','POLY_AREA']
    patch_list = []
    cur = da.SearchCursor(multi_shp,fields)
    for row in cur:
        patch_list.append((row[0],row[1]))
    del cur
    patch_list.sort(key = lambda a: a[1]) # 按照面积的从大到小排序
    main_patch_buffer = patch_list[0][0].buffer(1000)
    # 将斑块都拿出来当做单独要素重新命名
