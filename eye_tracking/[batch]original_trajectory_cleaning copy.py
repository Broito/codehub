...
# %% 开始遍历表
tables = os.listdir(r'./原始导出文件/基础表')
for table in tables:
    start_time = datetime.now()
    print(f'{table}开始计算：')
    df = pd.read_csv(f'./原始导出文件/基础表/{table}',sep='\t')
    ...
    df.to_excel(f"./原始导出文件/原始数据清洗/{table[:-4]}.xlsx",encoding = 'utf8')
    ...
    # # 导出总表，备份一下
    # total_df.to_excel('答案总表_v67_眼动语义汇总.xlsx')
    print(f'{table}计算结束：共耗时{(datetime.now()-start_time).seconds/60}分钟')
    print('-----------------------------\n')
