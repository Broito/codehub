...
roadsh="C:\\GPS\\speed\\data.gdb\\road"
def insert3_point():
    "首先插入前三个点"
...
for day in range(4,6):#运行7天的数据
    dataPath="C:\\GPS\\speed\\matchResult\\070%d.gdb"%(day)
    ...
    for nearFile in fileName:
        try:
            ...
            arcpy.CreateTable_management(dataPath, newtable, "C:\\GPS\\speed\\data.gdb\\template", "")
            # 按得分从小到大排列，全局只搜索一次，变成矩阵
            curAll = sorted(arcpy.da.SearchCursor(neartable, ["IN_FID","NEAR_FID","NEAR_X","NEAR_Y","NEAR_DIST","Field2","Field7","distance","road_score"]),key=lambda a_list: a_list[8])
            curLine=sorted(arcpy.da.SearchCursor(roadsh,["SHAPE@","OID@"]),key=lambda a_list: a_list[1])#搜索道路数据
