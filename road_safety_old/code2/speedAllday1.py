...
def get_result(time1,time2,sheetName):
    ...
    sheet.write(0, 4, "passTimes" + sheetName)  #通行次数
    ...
    for work in workSpace:#work是天数。18,19...
        env.workspace = "C:\\GPS\\speed\\matchResult\\07061.gdb"
        gap1="2016-07-06 00:00:00"
        timeGap1=datetime.datetime.strptime(gap1, "%Y-%m-%d %H:%M:%S")#时间间隔
        gap2="2016-07-06 23:59:59"
        ...
        for fname in fileName:
            ...
            for curRow in curf:
                if curRow[2]>=timeGap1 and curRow[2]<=timeGap2:
                    time0.append(curRow)
            for i in range(0,len(time0)-1):
                if i ==0:
                    rid = time0[0][1]
                    passTime[rid] = passTime[rid] + 1
                if time0[i+1][1]!=time0[i][1] and (time0[i+1][2]-time0[i][2]).seconds>3600:
                    rid = int(time0[i+1][1])
                    passTime[rid] = passTime[rid]+1
            if len(time0)>0:
    ...
    for i in range(len(roadSpeed)):#i=0代表路段id为1
        ...
        if carNum!=0:
            ...
            sheet.write(rowID, 4, passTime[i+1])
            rowID += 1
    saveFile="C:\\GPS\\speedALL24.xls"
    workbook.save(saveFile)  # 保存数据
roadID=[]#道路ID
for i in range(1,43931):
    roadID.append(i)
workSpace=[]#天数
for i in range(18,19):#1号至24号所有数据求速度均值
    workSpace.append(i)
workbook = xlwt.Workbook(encoding = 'utf-8')
for i in range(0,1):#执行24个时间段的
    sheetName="%d_%d"%(i,i+1)
    get_result(0,23,sheetName)#时间间隔0-2，工作表名称
    print "%d-%d时间段完成"%(i,i+2)
