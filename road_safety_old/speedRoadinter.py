...
#将每个时段的速度链接到道路上，没有速度的路段使用同等道路等级平均速度代替
def excel_table():
    "0_6小时段excel转换成table，速度有m/s转换成km/h"
    excelPath="F:\\trafficdata\\speed\\speed4_6.xls"#
    speedTable = "speedTableinter"#创建的表格
...
def speed_avg():
    ...
    road_osm = "road_speedinter"  # 道路数据,24个时间间隔
    workbook = xlwt.Workbook(encoding = 'utf-8')#
    sheet = workbook.add_sheet("roadspeed")
    fieldList=["roadgrade","roadSpeed4_6","roadSpeed12_14"]
    sheet.write(0, 0,"roadgrade")
    sheet.write(0,1, "roadSpeed4_6")
    sheet.write(0, 2, "roadSpeed12_14")
    #sheet.write(0, 3, "roadSpeed17_19")
    ...
    for road in roadType :
        ...
        for speedid in fieldList[1:]:#从0_1时开始,speedid=roadSpeed0_1
            ...
            print speedid
        rowID+=1
        workbook.save("F:\\trafficdata\\speed\\roadTypeSpeedinter.xls")  # 保存数据
        print road,"完成"
def speed_all():
    "没有速度的路段使用同等道路等级平均速度代替"
    road_osm = "road_speedinter"  # 道路数据,3个时间间隔
    excelPath="F:\\trafficdata\\speed\\roadTypeSpeedinter.xls"#获得不同道路类型平均速度
    excelFile = xlrd.open_workbook(excelPath)
    excel = excelFile.sheet_by_name("roadspeed")
    avgSpeed=[]#[[],[],[],[]],长度为4，每项3个值。不同道路类型各时段平均速度,快速路、主干道、次干道、支路
    nrows = excel.nrows  # 行数
    for rowid in range(1, nrows):
        rowarray = excel.row_values(rowid)  # 获取整行值，返回数组
        avgSpeed.append(rowarray[1:])
    fieldList=["roadgrade","roadSpeed4_6","roadSpeed12_14"]#["roadgrade","roadSpeed0_1"...]
