...
def get_result(time1, time2, sheetName):
    ...
    for i in range(1, 1372):
        roadSpeed.append([])
        instantSpeed.append([])
    for work in workSpace:  # work是天数。18,19...
        env.workspace = "F:\\trafficdata\\speed\\matchResult\\201504%d.gdb" % (work)
        gap1 = "2015-04-%d %d:00:00" % (work, time1)
        timeGap1 = datetime.datetime.strptime(gap1, "%Y-%m-%d %H:%M:%S")  # 时间间隔
        gap2 = "2015-04-%d %d:59:59" % (work, time2)
    ...
    for i in range(len(roadSpeed)):  # i=0代表路段id为1
        ...
        if carNum > 2:  # 车辆数至少3辆才有说服力
            ...
            rowID += 1
            saveFile = "F:\\trafficdata\\speed\\speed12_14.xls"
...
for i in range(1, 1372):
    roadID.append(i)
workSpace = []  # 天数
for i in range(18, 25):  # 1号至24号所有数据求速度均值
    workSpace.append(i)
workbook = xlwt.Workbook(encoding='utf-8')
get_result(12, 13, "12_14")  # 时间间隔4-6，工作表名称
