...
#     col+=1
sheet.write(0,1,'density8_9')
sheet.write(0,2,'density9_10')
sheet.write(0,3,'density17_18')
sheet.write(0,4,'density18_19')
rowID=1
...
networkND = "network\\network_ND"#网络数据集
centerRoad = "center_road_mor_even"#中心点数据
seardis = 200 #真实的搜索半径
bandWidth = "200"  # 在服务区工具里，85米搜索半径相当于100米
curCenter = sorted(arcpy.da.SearchCursor(centerRoad, ["SHAPE@", "newoid"],"""[newoid] >2038 AND [newoid]<2400"""), key=lambda a_list: a_list[1])  # 搜索道路中心点数据
for point in curCenter:
    ...
    del curJoin
    print point[1]
    workbook.save(r"E:\FloatVehicle2019\speed_distribution\200m_mor_even\cluster_mor_even.xls")  # 保存数据
    # except:
